  ...
  return (
    <div className="w-full max-w-md rounded-lg border border-[#30363d] bg-[#161b22] p-8 shadow-2xl">
      <header className="text-center">
        ...
        <h2 className="mt-6 text-xl font-bold text-[#f0f6fc]">Sign in to your account</h2>
      </header>
      ...
      <footer className="mt-8 border-t border-[#30363d] pt-6 text-center text-sm text-[#8b949e]">
        ...
        <Link href={`/auth/sign-up?callbackUrl=${encodeURIComponent(callbackUrl)}`} className="font-semibold text-[#58a6ff] hover:underline">
          Sign Up
        </Link>
      </footer>
    </div>
...
export default function SignInPage() {
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#0d1117] px-4 text-white">
      <Suspense fallback={<div className="text-[#8b949e] text-sm flex items-center gap-2"><Loader2 className="h-4 w-4 animate-spin" /> Loading sign in...</div>}>
        <SignInForm />
      </Suspense>
    </main>
  );
